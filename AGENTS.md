...
Read this file **first** before making architectural changes in fast-next.

fast-next is a **product-agnostic full-stack foundation** (Next.js + FastAPI + PostgreSQL + Redis/ARQ). Your job is to implement features **inside** the existing architecture — not redesign it.
...
| Where does the feature belong? | `backend/app/modules/apps/<name>/` + `frontend/src/lib/modules/apps/<name>/` + `frontend/src/app/(dashboard)/` |
| Is there a similar feature? | Inspect `modules/apps/` — reuse patterns |
| Should I scaffold? | Yes for new app modules: `__ctrl__\fast-next-ctrl.bat app create <name>` |
...
| How is the Next.js frontend structured? | `frontend/src/lib/modules/apps/<name>/api.ts` + `frontend/src/app/(dashboard)/<path>/page.tsx` |
...
| How to run the project? | `__ctrl__\fast-next-ctrl.bat dev run all` — see [cli.md](docs/cli.md) |
...
7. Inspect the **canonical example** at `backend/app/modules/apps/sample/` and `frontend/src/lib/modules/apps/sample/`.
...
2. Implement → run relevant tests via `__ctrl__` (`test all` for the touched surface) → only then mark `done`.
...
```bat
__ctrl__\fast-next-ctrl.bat app create myfeature
```
...
fast-next uses the **simple approach**: services raise `HTTPException` for business/API errors.
...
- Run: `__ctrl__\fast-next-ctrl.bat test all`
...
- Frontend API URL: `NEXT_PUBLIC_API_BASE_URL` (see `frontend/src/lib/config/backend.ts`).
...
```bat
__ctrl__\fast-next-ctrl.bat dev run all          # full runtime (Redis + ARQ worker)
__ctrl__\fast-next-ctrl.bat dev run all --slim   # slim runtime (no Redis/worker)
__ctrl__\fast-next-ctrl.bat app create myfeature  # scaffold new module
...
- Do **not** turn fast-next into a product-specific template (AI app, CMS, SaaS, e-commerce, etc.).
...
FoxG has separate architecture rules (admin CRUD, RBAC, custom responses). **Do not copy those into fast-next core.** Use [docs/conventions.md](docs/conventions.md) for fast-next-specific naming and boundaries.
...
frontend/src/app/(dashboard)/sample/notes/page.tsx  → Next.js UI
