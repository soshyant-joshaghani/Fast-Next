...
if not defined PY (
  echo [fast-next-ctrl] Python 3.10+ not found. Install from python.org or enable the py launcher.
  pause
  exit /b 1
)

if not exist ".venv\Scripts\python.exe" (
  echo [fast-next-ctrl] Creating .venv and installing requirements...
  %PY% -m venv .venv
  if errorlevel 1 (
