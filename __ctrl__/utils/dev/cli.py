"""fast-next-ctrl dev — run/stop local Docker Desktop / host app stacks."""
...
# __ctrl__ lives under fast-next/; the compose + apps root is the parent.
PROJECT = ROOT.parent
...
COMPOSE_PROJECT_NAME = "fast-next-dev"
...
APP_PORTS = (
    ("fast-next-backend", 8000),
    ("fast-next-worker", None),
    ("fast-next-frontend", FRONTEND_DEV_PORT),
)
...
def _python_for_venv() -> list[str]:
    """Argv prefix to create a venv (Windows Store `python` stub is not usable)."""
    if sys.platform == "win32":
        if shutil.which("py"):
            return ["py", "-3"]
...
def _setup_local(*, force_install: bool = False) -> int:
    ...
    if not venv.is_dir():
        ...
        if not shutil.which(venv_cmd[0]):
            print(f"error: '{venv_cmd[0]}' not found on PATH", file=sys.stderr)
            return 1
        print("[fast-next] Creating Python venv at .venv ...")
        proc = subprocess.run([*venv_cmd, "-m", "venv", str(venv)], cwd=PROJECT)
        if proc.returncode != 0:
            return proc.returncode
        created = True
    ...
    if created or force_install:
        print("[fast-next] Installing Python requirements...")
        proc = subprocess.run(
    ...
    if not (PROJECT / "node_modules").is_dir() or force_install:
        print("[fast-next] Installing npm dependencies at root ...")
...
def _run_migrations() -> int:
    ...
    print("[fast-next] Running migrations + seed (local prestart)...")
    for cmd in steps:
        print(f"  {' '.join(cmd)}")
    ...
    print("[fast-next] Migrations + seed complete.")
    return 0
...
def _start_infra(*, slim: bool = False) -> int:
    ...
    if code != 0:
        return code
    # Reload Traefik so dynamic.dev.yml edits apply without restarting db/redis.
    code = _compose("up", "-d", "--force-recreate", "--no-deps", "proxy")
    if code != 0:
        return code

    print("[fast-next] Waiting for Postgres...")
    if _wait_postgres("db") != 0:
        return 1

    if not slim:
        print("[fast-next] Waiting for Redis...")
        if _wait_redis() != 0:
            return 1
...
def _start_named_console(
    ...
    *,
    ...
    """Open a titled console like start-fast-next-dev.bat (`start "title" cmd /k …`).
...
def _start_apps(*, slim: bool = False) -> int:
    ...
    print(f"[fast-next] Starting apps ({profile}): backend + frontend" + ("" if slim else " + worker"))
    if sys.platform == "win32":
    ...
    if not slim:
        print("Worker: fast-next-worker (ARQ)")
    print("Stop with: fast-next-ctrl.bat dev stop all")
    return 0


def _stop_apps() -> int:
    print("[fast-next] Stopping host apps (backend / frontend)...")
    killed: set[int] = set()
...
def _compose_down(*, volumes: bool = False) -> int:
    ...
    if volumes:
        cmd.append("-v")
        print("[fast-next] compose down -v  (WIPES VOLUMES)")
    code = _compose(*cmd)
    if code != 0:
        return code
    if volumes:
...
def _dev_help(args: argparse.Namespace) -> int:
    _ = args
    print(
        "usage: fast-next-ctrl.bat dev {run|start,stop,down,purge,reset} {infra,apps,all}\n"
        "\n"
        "examples:\n"
        "  fast-next-ctrl.bat dev run all\n"
        "  fast-next-ctrl.bat dev run all --slim\n"
        "  fast-next-ctrl.bat dev start all\n"
        "  fast-next-ctrl.bat dev stop apps\n"
...
def cmd_setup_local(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "force", False):
        venv = PROJECT / ".venv"
        if venv.is_dir():
            print("[fast-next] Removing existing .venv (--force)...")
            shutil.rmtree(venv)
        nm = PROJECT / "node_modules"
        if nm.is_dir():
            print("[fast-next] Removing node_modules (--force)...")
            shutil.rmtree(nm)
