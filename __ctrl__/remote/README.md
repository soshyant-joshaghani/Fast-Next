...
laptop CLI. Prefer **`fast-next-ctrl`** from your machine when possible.

| Script | CLI equivalent |
|--------|----------------|
| `start-prod.*` | `fast-next-ctrl start` (SSH) or `prod start` (local) |
...
| `setup-ubuntu.sh` | Prefer `fast-next-ctrl setup` from laptop |
