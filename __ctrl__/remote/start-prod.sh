...
for img in fast-next-backend:prod fast-next-frontend:prod; do
...
#   FAST_NEXT_STORAGE=external bash __ctrl__/remote/start-prod.sh
if [[ -f compose.minio.yml && "${FAST_NEXT_STORAGE:-bundled}" != "external" ]]; then
...
echo
echo "fast-next production stack started (includes ARQ worker)."
if [[ -f compose.minio.yml && "${FAST_NEXT_STORAGE:-bundled}" != "external" ]]; then
  ...
  echo "Object storage: external (FAST_NEXT_STORAGE=external — no compose.minio.yml)."
fi
