#!/usr/bin/env python3
"""fast-next SSH control — setup, clone, env, start/stop, update/reset, ACME.

Standalone (not foxg-ctrl). Run from this folder: fast-next-ctrl.bat …
"""
...
def _add_target(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        ...
        nargs="?",
        default="fast-next",
        help="server id (default: fast-next) or 'all'",
    )
    p.add_argument(
        "--all-disabled",
        action="store_true",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="fast-next-ctrl",
        description=(
            "Single CLI for fast-next: local dev, tests, prod helpers, and SSH VM ops."
        ),
        ...
        epilog=(
            ...
            "\n"
            "SSH (VM):\n"
            "  fast-next-ctrl.bat setup | clone | env | start | stop | update\n"
            "  fast-next-ctrl.bat backup-acme | status | connect"
        ),
    )
    ...
    for name, help_, fn in [
        ...
        ("clone", "Clone / pull the fast-next repo on the VM", cmd_clone),
...
def _repl() -> int:
    parser = build_parser()
    echo("fast-next-ctrl> (setup-local | dev run all | test all | list | quit)")
    while True:
        try:
            line = input("fast-next-ctrl> ").strip()
