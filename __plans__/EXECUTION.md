...
| Architecture | Keep Fast-Next’s Router → Service → Repository → PostgreSQL flow, auth, Alembic whitelist, cache helpers, `__ctrl__`, and full/slim profiles. |
| Styling | Tailwind + local shadcn/ui primitives only. No component-local CSS. |
...
- Repository identifies as the product, not Fast-Next.
