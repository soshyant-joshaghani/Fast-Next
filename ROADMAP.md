# fast-next — AI & Developer Foundation Roadmap

## 1. Project Vision

fast-next is an opinionated, modular, full-stack foundation designed to help developers go from an idea to a deployable service with minimal infrastructure decisions.

The goal is NOT to build a template for a specific type of product.

fast-next must remain product-agnostic.
...
The difference between these products should primarily be the modules implemented on top of fast-next, not a completely different project architecture.

The core philosophy is:

> Python backend + TypeScript frontend = Full Stack

A developer should be able to use Python for the backend, business logic, database layer, background processing, testing, and deployment workflow — and TypeScript/React for a modern, ecosystem-rich frontend.

Next.js provides the application UI.
...
fast-next is intended to be a launchpad, not a finished product.

---

# 2. Primary Goal

The primary goal is to make fast-next an excellent foundation for AI-assisted development and Vibe Coding.
...
All of these should be possible applications built on top of fast-next.
...
fast-next intentionally makes architectural decisions.
...
Conceptually:
    ...
    frontend/src/app/(dashboard)/<path>/page.tsx
...
Next.js is a first-class part of fast-next.

The frontend should not be treated as an afterthought.

fast-next pairs a Python backend with a modern TypeScript frontend — giving access to the full npm ecosystem (Three.js, Babylon.js, charting libraries, etc.) while keeping backend conventions in Python.
...
Redis and ARQ are part of the official fast-next infrastructure.

They should NOT be described as "optional technology" or as an unrelated add-on.

fast-next officially supports two runtime profiles:
...
fast-next also officially supports a lightweight development/runtime mode.
...
> Full is the complete fast-next runtime.

> Slim is the lightweight fast-next runtime.

Slim mode is NOT a workaround.

Slim mode is NOT a fallback.

Slim mode is an official supported operating mode of fast-next.
...
This document must explain how AI agents are expected to work inside fast-next.
...
The AI should not redesign fast-next simply because it personally prefers another architecture.

---

# 9. Canonical Example Module

fast-next should contain one intentionally simple but complete example module.
...
> "This is how a feature is implemented in fast-next."
...
fast-next should remain usable from the first local development command through production deployment.
...
> Does this make fast-next a better general-purpose foundation for developers and AI agents?
...
Product-specific functionality belongs in application modules, not the fast-next core.

---

# 22. Acceptance Criteria for fast-next

fast-next should eventually satisfy the following scenario:
...
fast-next should feel like a launchpad.
