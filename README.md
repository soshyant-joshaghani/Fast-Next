[![](https://github.com/soshyant-joshaghani/Fast-Next/raw/main/Fast-System.png?raw=true)](https://github.com/soshyant-joshaghani/Fast-Next)

[![](https://github.com/soshyant-joshaghani/Fast-Next/raw/main/Fast-Next.png?raw=true)](https://github.com/soshyant-joshaghani/Fast-Next)
...
Fast-Next is a **product-agnostic launchpad** — not a finished product and not a domain-specific template. Use the same foundation for dashboards, CRUD apps, APIs, SaaS products, 3D/WebGL experiences, background-processing services, or AI-powered applications. The difference is the **modules you add**, not a different project architecture.

**Documentation:** [AGENTS.md](AGENTS.md) · [ROADMAP.md](ROADMAP.md) · [docs/](docs/)

---

## What Fast-Next provides

Fast-Next answers repeated architectural and infrastructure questions before development starts:

| Pillar | What you get |
|--------|--------------|
| **Architecture** | Modular backend (FastAPI) + frontend (Next.js) with clear layer boundaries |
...
You provide product requirements, features, business rules, UI needs, and integrations. Fast-Next provides structure, conventions, lifecycle control, background processing, testing, and deployment.

> **Python backend + TypeScript frontend = Full Stack.** Next.js + shadcn/ui for UI. FastAPI for API. PostgreSQL for data. Redis + ARQ for background jobs. `__ctrl__` for the workflow.

---

## Why it exists

Building a service usually means re-deciding the same things: project layout, module boundaries, database migrations, auth, background jobs, testing, and deployment. Fast-Next makes those decisions once — so you and AI agents can focus on the product.

```text
Idea
  ↓
Fast-Next (foundation + guardrails)
  ↓
...
Fast-Next does not build products for you. It provides the runway; AI helps implement features **inside** the existing architecture.
...
| Layer | Tech | Dev URL |
|-------|------|---------|
| Frontend | Next.js + React + shadcn/ui + TypeScript + Tailwind | http://dashboard.localhost |
...
|---------|---------|----------|
| **Full** | `dev run all` | Postgres, Redis, ARQ worker, Traefik, Adminer, uvicorn, Next.js |
| **Slim** | `dev run all --slim` | Postgres, Traefik, Adminer, uvicorn, Next.js (no Redis / no worker) |
...
```bat
__ctrl__\fast-next-ctrl.bat setup-local
__ctrl__\fast-next-ctrl.bat dev run all
```
...
| Direct Next.js | http://localhost:3100 |
| Direct API | http://localhost:8000/docs |

Linux/mac:

```bash
chmod +x __ctrl__/fast-next-ctrl.sh
__ctrl__/fast-next-ctrl.sh setup-local
__ctrl__/fast-next-ctrl.sh dev run all
```

Stop: `__ctrl__\fast-next-ctrl.bat dev stop all`
...
2. Scaffold (recommended): `__ctrl__\fast-next-ctrl.bat app create myfeature`
3. Backend: `backend/app/modules/apps/<name>/`
4. Frontend client: `frontend/src/lib/modules/apps/<name>/api.ts`
5. Next.js route: `frontend/src/app/(dashboard)/<path>/page.tsx`
...
│   ├── components/           # shadcn/ui + layout shell
│   └── lib/
│       ├── config/           # API_BASE_URL
...
```bat
__ctrl__\fast-next-ctrl.bat test all
```
...
```bat
__ctrl__\fast-next-ctrl.bat setup
__ctrl__\fast-next-ctrl.bat clone
__ctrl__\fast-next-ctrl.bat env
__ctrl__\fast-next-ctrl.bat start
```
...
| Kit | Role |
|-----|------|
| **Fast-Next** (this repo) | General-purpose foundation (Next.js UI) |
...
`NEXT_PUBLIC_API_BASE_URL` — where the frontend calls FastAPI (browser).
