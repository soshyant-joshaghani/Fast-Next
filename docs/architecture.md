...
fast-next is a modular full-stack foundation. This document describes how the pieces fit together.
...
fast-next supports two official dev/runtime modes:
...
Shared infrastructure used by every application: authentication, users. These ship with fast-next and are not product-specific.

Uses `crud.py` in the users submodule (predates the app-module convention).

### App modules (`modules/apps/<name>/`)

Product features you build on top of fast-next. Each module owns its backend and frontend code.
...
- `lib/modules/base/auth/LoginForm.tsx` — login UI
- `lib/modules/base/auth-context.tsx` — session persistence
...
- Frontend stores token via `AuthProvider` in `lib/modules/base/auth-context.tsx`
...
```bat
__ctrl__\fast-next-ctrl.bat test backend
```

Frontend tests use Vitest:

```bat
__ctrl__\fast-next-ctrl.bat test frontend
```
...
**Core** (fast-next foundation): auth, database, migrations, CLI, deployment, testing infrastructure, modular conventions.
...
fast-next intentionally uses the **simple approach**: services raise `HTTPException` for business errors. No separate application-exception hierarchy.
...
fast-next does **not** ship a storage abstraction yet. When applications need uploads or generated files:
