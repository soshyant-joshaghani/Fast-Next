...
fast-next is designed so you focus on product requirements, not repeated architecture decisions:
...
fast-next provides: structure, conventions, control CLI, testing, deployment path.
...
- **UI:** Vite HMR via `npm run dev` on port 5173 (host)
...
Install dependencies after clone: `__ctrl__\fast-next-ctrl.bat setup-local` or `npm install` from repo root.
