...
Naming, layers, and cross-module rules for fast-next. Follow these in app modules; platform code (`base/`, `system/`) sets the baseline.
...
fast-next uses **standard FastAPI**, not a custom envelope.
...
**Intentional decision:** fast-next uses the simple approach — services raise `HTTPException` directly. Do not introduce a separate application-exception hierarchy unless a product module explicitly requires it.

Do **not** introduce FoxG-style `{ "code": "SUCCESS", "data": {}, "meta": {} }` wrappers unless an app module explicitly requires that contract.

## Cross-module relations

fast-next modules: `base/` (platform), `system/`, `apps/<feature>/`.
...
These belong in **app modules**, not fast-next foundation docs or core code:
