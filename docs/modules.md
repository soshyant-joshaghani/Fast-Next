...
Before creating a new pattern, inspect the **canonical `sample` module** (notes CRUD). It is the reference implementation for how features are structured in fast-next.
...
- Use `fetch` with auth headers from `@/lib/modules/base/auth-context`
