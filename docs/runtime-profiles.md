...
fast-next officially supports two **development** runtime profiles. Both are valid — neither is a workaround or degraded mode.

Redis and ARQ are part of the full fast-next runtime. Slim is an official lightweight profile for apps that do not need background processing — not because Redis is "optional infrastructure."
...
**Apps (host):** uvicorn (API :8000), ARQ worker, Vite (dashboard :5173)

Use when the application needs background jobs, or when you want the complete fast-next stack locally.

## Slim runtime

```bat
__ctrl__\fast-next-ctrl.bat dev run all --slim
```
...
Traefik routes `api.localhost` → :8000 and `dashboard.localhost` → :5173.
